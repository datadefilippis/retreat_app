      ...
      <div className="max-w-6xl mx-auto px-4">
        <div className="relative -mt-12 sm:-mt-14 rounded-2xl border border-gray-200
          ...
          <div className="flex flex-wrap items-center gap-2 mt-4">
            ...
            {/* RV7 (founder 10/9: «più in evidenza, ma lean») — la voce
                Recensioni resta nella fila dei badge, un filo più marcata
                (bordo oro, voto vero), e al clic scorre a #recensioni; dalla
                pagina intervista porta al profilo. C'è anche a zero. */}
            <button type="button" data-testid="reviews-cta"
                    ...
                    className="inline-flex items-center gap-1.5 rounded-full border border-[#c9b37e] bg-white px-3 py-1 text-[12px] font-semibold text-foreground shadow-sm transition-colors hover:bg-[#fbf7ee]">
              <span className="text-[#c9a24d]" aria-hidden>★</span>
              {rs?.count > 0
                ? <span>{rs.avg} · {t('landings:reviews.countShort', { count: rs.count, defaultValue: '{{count}} recensioni' })}</span>
                : <span>{t('landings:reviews.title', { defaultValue: 'Recensioni' })} · scrivi la prima</span>}
