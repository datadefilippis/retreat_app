      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-6 space-y-5">
        ...
        {activeTab === 'regole' && (
          <div className="space-y-4">
            ...
            {/* AP-L — requisiti e condizioni del servizio (terms_content,
                F4) promossi QUI nel passo condizioni: prima erano sepolti
                nel passo publish. Al checkout finiscono nella checkbox
                dinamica "Accetto le condizioni di {operatore}" insieme
                alla politica di cancellazione qui sopra. */}
            <div className="rounded-xl border border-gray-200 bg-white p-5">
              <p className="text-sm font-semibold text-gray-900">
                {t('wizards.event.regole.requisitiTitle', { defaultValue: 'Requisiti e condizioni del servizio' })}
              </p>
              <p className="text-xs text-gray-500 mt-0.5 mb-3">
                {t('wizards.event.regole.requisitiDesc', { defaultValue: 'Cosa deve sapere o dichiarare chi partecipa. Se compili questo campo, al checkout compare una casella di accettazione dedicata con questo testo.' })}
              </p>
              <textarea
                value={termsContent}
                onChange={e => setTermsContent(e.target.value)}
                rows={5} maxLength={20000}
                placeholder={t('wizards.event.regole.requisitiPlaceholder', { defaultValue: "Es. dichiarazione di assenza di controindicazioni mediche, requisiti di eta', cosa portare…" })}
                data-testid="wizard-requisiti"
        ...
        {activeTab === 'publish' && (
          ...
          />
            ...
            {/* F4 Onda 11 — l'editor dei termini per-evento e' stato
                promosso nel PASSO 3 "Regole e caparra" come "Requisiti e
                condizioni del servizio" (AP-L): un solo posto, accanto
