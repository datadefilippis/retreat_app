...
// toward end customers; Aurya is Data Processor). These endpoints are
// PUBLIC. PS5 (30/7/2026): accettano un ``lang`` opzionale — con lang
// valido il documento (autogenerato, o custom se il merchant ha
// scritto quella lingua) si serve nella lingua dell'utente; fa
// comunque fede la versione in ``binding_locale`` (la lingua di
// riferimento dello store), che l'envelope espone sempre.

/**
 * Fetch the merchant's Privacy Policy for a storefront.
 *
 * Returned envelope (always 200 when slug is valid):
 *   {
 *     content: string,                  // markdown ("" only on render failure)
 *     display_locale: "it"|"en"|"de"|"fr"|null,   // lingua SERVITA
 *     binding_locale: "it"|"en"|"de"|"fr"|null,   // lingua che fa fede
 ...
 * @param {"it"|"en"|"de"|"fr"} [locale] — lingua utente (PS5)
 */
export async function fetchStorefrontPrivacy(slug, locale) {
  if (!slug) throw new Error('fetchStorefrontPrivacy: slug required');
  const params = locale ? `?lang=${encodeURIComponent(locale)}` : '';
  const url = `${API_BASE}/api/legal/storefront/${encodeURIComponent(slug)}/privacy${params}`;
  const res = await axios.get(url, { headers: { Accept: 'application/json' } });
  return res.data;
}

/**
 * Fetch the merchant's Terms of Service for a storefront.
 * Same envelope shape as fetchStorefrontPrivacy with doc_type="terms".
 */
export async function fetchStorefrontTerms(slug, locale) {
  if (!slug) throw new Error('fetchStorefrontTerms: slug required');
  const params = locale ? `?lang=${encodeURIComponent(locale)}` : '';
  const url = `${API_BASE}/api/legal/storefront/${encodeURIComponent(slug)}/terms${params}`;
  const res = await axios.get(url, { headers: { Accept: 'application/json' } });
  return res.data;
}
