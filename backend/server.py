...
async def sitemap_xml():
    """LC1 — /sitemap.xml è l'INDICE phase-aware di routers/seo.py.

    In produzione nginx riscrive /sitemap*.xml sugli endpoint
    /api/public/sitemap*.xml, quindi questa rotta serve solo dove non
    c'è il proxy (dev, test): deve dire la stessa cosa. Prima serviva
    la sitemap monolitica di Fase 5 — sole URL /ritiri, che in fase
    rete rispondono 404 — mentre robots.txt la dichiarava ai crawler."""
    from fastapi.responses import Response
    from routers.seo import build_index
    return Response(build_index(), media_type="application/xml")
