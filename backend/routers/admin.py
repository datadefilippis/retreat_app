...
# ════════════════════════════════════════════════════════════════════════════
# Wave 8B — AI Governance: budgets + kill switch
# ════════════════════════════════════════════════════════════════════════════

from models.ai_budget import (  # noqa: E402
    AIBudgetCreate, AIBudgetUpdate, BUDGET_SCOPES, BUDGET_PERIODS,
)
from repositories import ai_budget_repository  # noqa: E402
from services.llm.budget_guard import (  # noqa: E402
    _read_kill_switch, write_kill_switch,
)
...
@router.get(
    "/ai-budgets",
    summary="List AI budgets — Wave 8B",
    description="List all AI budgets, optionally filtered by org/scope/is_active.",
)
async def admin_list_ai_budgets(
    organization_id: Optional[str] = Query(default=None),
    scope: Optional[str] = Query(default=None,
        description=f"One of: {', '.join(BUDGET_SCOPES)}"),
    is_active: Optional[bool] = Query(default=None),
    limit: int = Query(default=200, ge=1, le=1000),
    _: dict = Depends(require_system_admin),
):
    """Return budgets matching the filters, with current spend per budget.

    Each row carries ``current_spend_usd`` computed live from
    ai_usage_events within the budget's period window, so the admin UI
    can render progress bars without an extra round-trip.
    """
    rows = await ai_budget_repository.list_budgets(
        organization_id=organization_id, scope=scope,
        is_active=is_active, limit=limit,
    )
    # Annotate each row with current_spend_usd + period_start_iso for UI.
    enriched = []
    for b in rows:
        try:
            ...
            logging.warning("admin_list_ai_budgets: spend calc failed for %s: %s",
                            b.get("id"), exc)
            spend = None
        enriched.append({
            **b,
            "current_spend_usd": spend,
            "period_start_iso": ai_budget_repository.period_start_iso(b["period"]),
            "soft_limit_reached": (
                spend is not None and spend >= (b.get("soft_limit_usd") or 0)
            ),
            "hard_limit_reached": (
                spend is not None and spend >= (b.get("hard_limit_usd") or 0)
            ),
        })
    return {"budgets": enriched, "count": len(enriched)}


@router.post(
    "/ai-budgets",
    status_code=201,
    summary="Create or replace an AI budget — Wave 8B",
    description=(
        "Upsert keyed on (scope, scope_id, period). Re-posting the same "
        "tuple updates the limits + re-enables the budget."
    ),
)
async def admin_create_ai_budget(
    payload: AIBudgetCreate,
    current_user: dict = Depends(require_system_admin),
):
    if payload.scope not in BUDGET_SCOPES:
        raise HTTPException(status_code=400,
                            detail=f"scope must be one of {BUDGET_SCOPES}")
    if payload.period not in BUDGET_PERIODS:
        raise HTTPException(status_code=400,
                            detail=f"period must be one of {BUDGET_PERIODS}")
    if payload.hard_limit_usd < payload.soft_limit_usd:
        raise HTTPException(
            status_code=400,
            detail="hard_limit_usd must be >= soft_limit_usd",
        )
    doc = await ai_budget_repository.create_budget(
...
@router.patch(
    "/ai-budgets/{budget_id}",
    summary="Partial update of an AI budget — Wave 8B",
)
async def admin_update_ai_budget(
    budget_id: str,
    ...
    if (
        "soft_limit_usd" in updates and "hard_limit_usd" in updates
        and updates["hard_limit_usd"] < updates["soft_limit_usd"]
    ):
        raise HTTPException(
            status_code=400,
            detail="hard_limit_usd must be >= soft_limit_usd",
        )
    # Wave 10.C.6 — plumb the acting admin so the audit log can show who.
    actor = current_user.get("user_id") or current_user.get("id")
    doc = await ai_budget_repository.update_budget(
        budget_id, updates, actor=actor,
    )
    if doc is None:
...
@router.delete(
    "/ai-budgets/{budget_id}",
    summary="Delete an AI budget — Wave 8B",
)
async def admin_delete_ai_budget(
    budget_id: str,
    current_user: dict = Depends(require_system_admin),
):
    # Wave 10.C.6 — plumb the acting admin so the audit log can show who.
    actor = current_user.get("user_id") or current_user.get("id")
    deleted = await ai_budget_repository.delete_budget(budget_id, actor=actor)
    if not deleted:
        raise HTTPException(status_code=404, detail="Budget not found")
...
@router.get(
    ...
    summary="Read AI kill switch state — Wave 8B",
)
async def admin_get_kill_switch(_: dict = Depends(require_system_admin)):
    """Return the active kill-switch document (or defaults if never set)."""
    return await _read_kill_switch()


@router.post(
    "/ai-governance/kill-switch",
    summary="Update AI kill switch — Wave 8B",
    description=(
        "Atomic upsert of the platform-wide kill switch. "
        "Setting ai_enabled=false stops ALL Anthropic calls platform-wide. "
        "ai_throttle_pct in [0,100] randomly rejects that percentage of calls. "
        "Reason is REQUIRED for audit trail when disabling or throttling."
    ),
)
async def admin_set_kill_switch(
    ...
    """Body: {ai_enabled: bool, ai_throttle_pct: int (optional, default 0), reason: str}"""
    ai_enabled = bool(payload.get("ai_enabled", True))
    throttle = int(payload.get("ai_throttle_pct", 0))
    reason = (payload.get("reason") or "").strip() or None

    # Audit guard: any restrictive action requires a reason.
    if (not ai_enabled or throttle > 0) and not reason:
        raise HTTPException(
            status_code=400,
            detail="A reason is required when disabling or throttling AI.",
        )
    if throttle < 0 or throttle > 100:
        raise HTTPException(
            status_code=400,
            detail="ai_throttle_pct must be in [0, 100]",
        )

    activated_by = current_user.get("user_id") or current_user.get("id")
    new_state = await write_kill_switch(
        ai_enabled=ai_enabled, ai_throttle_pct=throttle,
...
#
# These four endpoints close the "I can't answer obvious questions in 3
# clicks" gap the Wave 10 audit surfaced on the governance dashboard:
#
#   - audit-log         → who toggled the kill switch / changed budgets?
#   - top-conversations → which single chat cost us most this week?
#   - conversations/{id} → round-by-round breakdown of one conversation
#   - failed-events     → list of recent errored AI calls (error_code != null)
# ════════════════════════════════════════════════════════════════════════════


@router.get(
    "/ai-governance/audit-log",
    summary="Governance mutations audit log — Wave 10.C.6",
    description=(
        "Lists kill-switch toggles and budget CRUD operations. Filters "
        "audit_logs where resource_type in ('ai_governance', 'ai_budget')."
    ),
)
async def admin_ai_governance_audit_log(
    limit: int = Query(default=100, ge=1, le=500),
    offset: int = Query(default=0, ge=0),
    _: dict = Depends(require_system_admin),
):
    """Recent governance audit entries, newest first."""
    from database import audit_logs_collection
    query = {"resource_type": {"$in": ["ai_governance", "ai_budget"]}}
...
@router.get(
    "/ai-usage/top-conversations",
    summary="Top N most expensive chat conversations — Wave 10.C.8",
    description=(
        "Aggregates ai_usage_events by conversation_id within the window, "
        "sorted by total cost desc. Powers the dashboard 'most expensive "
        "single conversation' drill-down."
    ),
)
async def admin_ai_usage_top_conversations(
    start_date: str = Query(..., description="ISO date inclusive"),
    end_date: str = Query(..., description="ISO date inclusive"),
    ...
    """Return the N most expensive conversations in the window."""
    from database import ai_usage_events_collection, organizations_collection, users_collection
    match: dict = {
        "module_key": "ai_assistant",
        "feature": "chat",
        "conversation_id": {"$ne": None, "$exists": True},
        "created_at": {
            "$gte": start_date,
            "$lte": end_date + "T23:59:59",
        },
    ...
    # Wave 10.C.2 — enrich with human names so the dashboard table is
    # readable. Tiny batch lookup (max 100 rows × 2 collections).
    if rows:
...
@router.get(
    ...
    summary="Round-by-round breakdown of one conversation — Wave 10.C.8",
)
async def admin_ai_usage_conversation_detail(
    conversation_id: str,
    _: dict = Depends(require_system_admin),
):
    """All events bearing this conversation_id, ordered by created_at.

    The frontend renders this as a "drill-into" modal: each row is one
    round-trip with tokens, cost, latency, and any error_code.
    """
    from database import ai_usage_events_collection
    ...
    total_cost = round(sum((r.get("cost_usd") or 0) for r in rows), 4)
    total_tokens = sum(
        (r.get("tokens_prompt") or 0) + (r.get("tokens_completion") or 0)
        for r in rows
    )
    return {
        "conversation_id": conversation_id,
...
@router.get(
    ...
    summary="Recent AI calls that errored — Wave 10.C.7",
    description=(
        "Lists events with error_code != null in the window. Today this "
        "is always empty (no call site populates error_code yet) — the "
        "endpoint exists so the dashboard panel is wired and ready."
    ),
)
async def admin_ai_usage_failed_events(
    start_date: str = Query(..., description="ISO date inclusive"),
    end_date: str = Query(..., description="ISO date inclusive"),
    org_id: Optional[str] = Query(default=None),
    limit: int = Query(default=100, ge=1, le=500),
    ...
    """Errored events in the window, newest first."""
    from database import ai_usage_events_collection
    match: dict = {
        "module_key": "ai_assistant",
    ...
    cursor = ai_usage_events_collection.find(match, {"_id": 0})
    cursor = cursor.sort([("created_at", -1)]).limit(limit)
    rows = await cursor.to_list(length=limit)

    # Rollup by error_code for a "what's broken right now" headline.
    by_code: dict = {}
    for r in rows:
