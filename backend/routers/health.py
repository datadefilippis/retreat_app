...
async def ai_health() -> Dict[str, Any]:
    """AI subsystem health: provider availability + circuit breaker state.

    Wave 5.4 (2026-05). Returns 200 with a JSON report:
        {
          "ok": bool,
          "provider": "anthropic",
    ...
    Used by:
      - Render / k8s alerting (treat ok=false as PagerDuty trigger)
      - System admin dashboard "is AI working right now"
      - Smoke E2E tests post-deploy

    Does NOT call Anthropic to verify the API key still works — that
    would burn tokens on every probe. is_available() checks only that
    the key+SDK are configured. The circuit breaker state reflects
    recent reality.
    """
    try:
        ...
        ok = configured and cb_state in ("CLOSED", "HALF_OPEN")
        ...
        return {
            ...
            "details": {
                "configured_note": (
                    "ANTHROPIC_API_KEY present + SDK importable"
                    if configured
                    else "ANTHROPIC_API_KEY missing or anthropic SDK not installed"
                ),
                "cb_note": {
                    "CLOSED": "Provider healthy.",
                    "OPEN": "Provider failing — fast-failing for cool-down.",
                    "HALF_OPEN": "Probing recovery (one request allowed).",
                }.get(cb_state, "unknown"),
            },
        }
    except Exception as e:
        logger.error("ai_health error: %s", e, exc_info=True)
        return {
            "ok": False,
