...
async def customer_signup(
    ...
    *, auto_login: bool = False, signup_slug: str | None = None,
    ...
    AP0 (2026-07-29, coerenza RS3): if the merchant has not published
    their docs yet, the signup PROCEEDS anyway — the storefront serves
    the autogenerated legal documents on /s/:slug/*, so there IS a
    binding text the customer saw and accepted. The consent snapshot is
    stamped with version "autogen:v0" (same sentinel order_creation_service
    uses for CG-5 order consents) and the store's effective display
    locale. The 400 remains ONLY when the store document does not exist
    at all (nothing served → nothing to bind to).
    """
    ...
    # CG-4: resolve store — the consent must bind to a real store doc.
    # AP0: publication is no longer required (autogen docs are served
    # and legally bind, RS3); only a MISSING store blocks signup.
    store_doc = None
    if signup_slug:
    ...
    legal_version = (
        current_version_string(store_doc)
        if legal_status in ("published", "stale_draft")
        else None
    )
    # AP0 (coerenza RS3) — merchant senza legal pubblicati: lo
    # storefront serve comunque i documenti autogenerati su /s/:slug/*,
    # quindi il consenso si timbra con la stessa versione sentinella
    # degli ordini (order_creation_service, CG-5): "autogen:v0".
    # partition(":") sotto la spezza in tag "autogen" + hash "v0",
    # coerente con i record consent_audit degli ordini.
    # Copre anche il caso difensivo published-ma-version-irrisolvibile
    # (prima era un 400): meglio un account con snapshot autogen che
    # un signup bloccato.
    legal_version = legal_version or "autogen:v0"
    # CG-3-Polish-3 — resolve via helper (the auto-cleanup unsets the
    # raw legacy field; reading it directly returns None and breaks
    # signup for any store edited after polish-2 shipped). The helper
    # already falls back to "it"; the `or "it"` is belt-and-braces.
    legal_display_locale = get_effective_display_locale(store_doc) or "it"
