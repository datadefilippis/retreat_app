...
Scope:
  ...
  - customer_signup REJECTS signup with 400 when:
      ...
      c) the store document does not exist
    (AP0 2026-07-29: "merchant not published yet" NO LONGER rejects —
    the signup proceeds with the autogen:v0 snapshot, RS3 coherence)
...
class TestCustomerSignupConsentEnforcement:
    ...
    @pytest.mark.asyncio
    async def test_signup_proceeds_with_autogen_when_merchant_not_published(self):
        """AP0 (2026-07-29, coerenza RS3): a merchant without published
        legal docs still serves the AUTOGENERATED documents on
        /s/:slug/* — so signup PROCEEDS and the consent snapshot is
        stamped with the same sentinel version the orders use
        ("autogen:v0"). Pre-AP0 this raised a 400 ("non ha ancora
        pubblicato"), which broke checkout-inline account creation on
        every not_configured store (the founder's bug)."""
        from services import customer_auth_service
        ...
        # Snapshot on the account doc = autogen sentinel + store locale
        assert captured_account["accepted_store_terms_version"] == "autogen:v0"
        assert captured_account["accepted_store_privacy_version"] == "autogen:v0"
        assert captured_account["accepted_store_terms_locale"] == "it"
        # Audit records use the same partition the orders use
        assert len(captured_audit) == 2
        for rec in captured_audit:
            assert rec["version_tag"] == "autogen"
            assert rec["version_hash"] == "v0"
            assert rec["source"] == "customer_signup"
